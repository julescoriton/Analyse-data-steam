...
def process_batches(parser, app_list, download_path, data_filename, index_filename,
    ...
    # len(batches) - 1 ==> because the loop is over the intervals
    for i in range(len(batches) - 1): 
        ...
        # Keep a track of the number of lines
        apps_written += len(app_data)
        

        # Keep a tack of the last written data 
...
url = "https://steamspy.com/api.php"
json_data = []
app_list_full = pd.DataFrame()
for i in range(51):
    parameters = {"request": "all", "page": str(i)}
    # request 'all' from steam spy page "i" and parse into dataframe
    json_data = get_request(url, parameters=parameters)
    steam_spy_all = pd.DataFrame.from_dict(json_data, orient='index')
    # generate sorted app_list from steamspy data
    app_list = steam_spy_all[['appid', 'name']].sort_values('appid').reset_index(drop=True)
    # export disabled to keep consistency across download sessions
    app_list.to_csv('./data/download/app_list'+str(i)+'.csv', index=False)
    app_list_info = pd.read_csv('./data/download/app_list'+str(i)+'.csv', sep= ',')
    app_list_full = app_list_full.append(app_list_info)
...
### Extract data from steamspy API ###
# ================================== #
